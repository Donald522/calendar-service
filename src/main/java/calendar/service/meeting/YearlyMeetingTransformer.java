...
@Component
public class YearlyMeetingTransformer implements MeetingTransformer {

  private final int recurrenceDuration;

  @Autowired
  public YearlyMeetingTransformer(@Value("${calendar.recurrence.duration.events:180}") int recurrenceDuration) {
  ...
  public List<Meeting> transform(Meeting meeting) {
    ...
    return IntStream.range(0, recurrenceDuration)
        .mapToObj(i -> meeting
            .withSubId(i + 1)
            .withFromTime(startFrom.plusYears(i))
            .withToTime(startTo.plusYears(i)))
        .collect(toList());
  }
}
